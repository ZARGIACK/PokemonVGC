...
// Ensure DB schema for favorites exists
  ...
  try {
    const rows = await q(`
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        ...
        AND COLUMN_NAME = 'pokemon_sid'
    `);
    const exists = rows && rows[0] && Number(rows[0].c) > 0;
    if (!exists) {
      await q('ALTER TABLE player ADD COLUMN pokemon_sid INT NULL');
      console.log('Added player.pokemon_sid column for favorites');
    }
  } catch (e) {
    console.error('ensureFavoriteColumn error:', e.message || e);
  }
}
...
// Get move details by code
app.get('/moves/:code', (req, res) => {
    const code = req.params.code;

    const sql = `
        SELECT code, name, type_name as type, power, accuracy, category, effect
        FROM moves
        WHERE code = ?
        LIMIT 1
    `;
    ...
    db.query(sql, [code], (err, results) => {
        if (err) return res.status(500).json({ error: 'Database error' });
        if (!results || results.length === 0) return res.status(404).json({ error: 'Move not found' });
        res.json(results[0]);
    });
});
...
app.post('/api/favorite', authMiddleware, async (req, res) => {
  try {
    ...
    // Ensure a player row exists, then set favorite
    const playerRows = await q('SELECT user_id FROM player WHERE user_id = ? LIMIT 1', [userId]);
    if (!playerRows || playerRows.length === 0) {
      await q('INSERT INTO player (user_id, pokemon_sid) VALUES (?, ?)', [userId, pokemonId]);
    } else {
...
app.delete('/api/favorite', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await q('UPDATE player SET pokemon_sid = NULL WHERE user_id = ?', [userId]);
    // If no player row existed, create one with NULL favorite to keep schema consistent
    if (!result || (typeof result.affectedRows !== 'number') || result.affectedRows === 0) {
      const playerRows = await q('SELECT user_id FROM player WHERE user_id = ? LIMIT 1', [userId]);
      if (!playerRows || playerRows.length === 0) {
        await q('INSERT INTO player (user_id, pokemon_sid) VALUES (?, NULL)', [userId]);
      }
    }
...
// Start server after ensuring schema is ready
(async () => {
